...
@Slf4j
@Component
public class PlaybackPreviousHandler implements RequestHandler {
    ...
    public boolean canHandle(HandlerInput input) {
        ...
        return playbackInfo.isInPlaybackSession() &&
                (input.matches(Predicates.requestType(PreviousCommandIssuedRequest.class))
                        || input.matches(Predicates.intentName("AMAZON.PreviousIntent")));
    }
    ...
    public Optional<Response> handle(HandlerInput input) {
        ...
        return audioPlayerController.playPrevious(input, playbackInfo, playbackSetting);
    }
}
